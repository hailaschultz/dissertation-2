...
```{r}
Zoop_env[Zoop_env < 0] <- NA
colnames(Zoop_env)
#remove rows with NA
Zoop_env<-Zoop_env[!is.na(Zoop_env$Code),]
Zoop_env<-Zoop_env[!is.na(Zoop_env$NITRATE_UMOL_L_Surface),]
Zoop_env<-Zoop_env[!is.na(Zoop_env$CTDSAL_PSS78_Surface),]
```

#remove rows with all zoooplankton NAs
```{r}
rows_to_keep <- apply(Zoop_env[, 19:230], 1, function(row) !all(is.na(row)))
...
2014 is similar to 2017
2014 is similar to 2018
2014 is similar to 2022
2015 is similar to 2018
2015 is similar to 2020
2015 is similar to 2022
2016 is similar to 2022
2017 is similar to 2020
2017 is similar to 2021
2017 is similar to 2022
2018 is similar to 2022
...
```{r}
data.scores$temp_deep = Zoop_env$CTDTMP_DEG_C_ITS90_Deep
data.scores$temp_surface = Zoop_env$CTDTMP_DEG_C_ITS90_Surface
data.scores$salinity_deep = Zoop_env$CTDSAL_PSS78_Deep
data.scores$salinity_surface = Zoop_env$CTDSAL_PSS78_Surface
data.scores$oxygen_deep = Zoop_env$OXYGEN_MG_L_1_Deep
data.scores$oxygen_surface = Zoop_env$OXYGEN_MG_L_1_Surface
data.scores$nitrate_deep = Zoop_env$NITRATE_UMOL_L_Deep
data.scores$nitrate_surface = Zoop_env$NITRATE_UMOL_L_Surface
...
env.scrs
```
nitrate is correlated with almost everything
## plot only April
