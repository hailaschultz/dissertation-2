...
# heatmap
```{r}
heatmap <- ggplot(Zoop_sub, aes(Genus.species,Code,fill = "zoop_density")) +
  geom_tile()
heatmap

library(pheatmap)
Zoop_sub_heat<-dcast(Zoop_sub, Code~ Genus.species,value.var = "zoop_density")
Zoop_sub_heat <- Zoop_sub_heat %>% replace(is.na(.), 0)
Zoop_sub_heat<-Zoop_sub_heat[rowSums(Zoop_sub_heat[])>0,]
pheatmap(Zoop_sub_heat)
...
indicator species
```{r}
inv = multipatt(RE2, Zoop_env$Month.x, func = "r.g", control = how(nperm=999))
summary(inv)
```
...
```{r}
dist<-vegdist(RE2, method='bray')
perm<-adonis2(dist~Year.x*STATION_NO, data=Zoop_env_april, permutations = 999, method="bray")
perm
```
indicator species
```{r}
inv = multipatt(RE2, Zoop_env_april$Year.x, func = "r.g", control = how(nperm=999))
summary(inv)
```
...
data.scores$Month = Zoop_env_april$Month.x
```
...
Zoo.env <- data.frame(matrix(ncol = 0, nrow=46))
colnames(data.scores)
Zoo.env$temp_deep<-as.numeric(data.scores$temp_deep)
Zoo.env$temp_surface<-as.numeric(data.scores$temp_surface)
Zoo.env$salinity_deep<-as.numeric(data.scores$salinity_deep)
...
Species_plot <- ggplot(data.scores, aes(x = NMDS1, y = NMDS2)) +geom_point(aes(colour = Station))+theme_bw()+theme(panel.grid.major = element_blank(),
  ...
  scale_fill_manual(values=c("#F8766D","#C49A00","#53B400","#00C094","#00B6EB","#A69AFF","#FB61D7"))+ geom_segment(data = spp.scrs,size=0.2,
               ...
               arrow = arrow(length = unit(0.08, "cm")), colour = "black")+
  geom_label_repel(data = spp.scrs, aes(x = NMDS1, y = NMDS2, label = Species),
                  size = 1.5, fontface="bold", fill="white", label.padding = unit(0.15, "lines"), box.padding = unit(0.16, "lines"), label.size = 0.05)+geom_segment(data = env.scrs,size=0.5,
               aes(x = 0, xend = NMDS1, y = 0, yend = NMDS2),
               arrow = arrow(length = unit(0.5, "cm")), colour = "black")+
  geom_label_repel(data = env.scrs, aes(x = NMDS1, y = NMDS2, label = Species),
